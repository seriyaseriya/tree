...
最初の試作では、木をクリックして丸太を獲得する機能だけを実装する。
...
4. ChoppingControllerが現在クールタイム中か確認する

5. ChoppingControllerがChoppingServiceを呼び出す

6. ChoppingServiceが獲得丸太量を計算する

7. ChoppingServiceが伐採結果を返す
...
クリッククールタイムや修繕ゲージなど、アプリ起動中の短い時間処理にはUnityの時間を使用してよい。
...
一時停止中も進める必要がある処理には、`Time.unscaledDeltaTime`を検討する。
...
初期段階では、ControllerがViewへ明示的に表示更新を要求する方式を採用してよい。
